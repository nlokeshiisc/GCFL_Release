...
class CRAIGStrategy(DataSelectionStrategy):
    ...
    def compute_score(self, model_params, idxs):
        ...
        subset_loader = torch.utils.data.DataLoader(trainset, batch_size=self.trainloader.batch_size*4, shuffle=False,
        ...
        if self.if_convex:
            for batch_idx, (inputs, targets, data_ids) in enumerate(subset_loader):
            ...
            for batch_idx, (inputs, targets, data_ids) in enumerate(subset_loader):
                ...
                if self.selection_type == 'PerBatch':
                    ...
                    self.N += inputs.size()[0]
                out, l1 = self.model(inputs, freeze=False, last=True)
                loss = self.loss(out, targets).sum()
                l0_grads = torch.autograd.grad(loss, l1)[0]
                #print(self.N)
                if self.linear_layer:
                    '''l0_expand = torch.repeat_interleave(l0_grads, embDim, dim=1)
                    #print(l0_expand.shape,l1.shape,l1.repeat(1, self.num_classes).shape,self.num_classes)
    ...
    def get_similarity_kernel(self):
        ...
        """
        for batch_idx, (inputs, targets, data_ids) in enumerate(self.trainloader):
            if batch_idx == 0:
                labels = targets
            else:
    ...
    def select(self, budget, model_params):
        ...
        if self.selection_type == 'PerClass':
            for i in range(self.num_classes):
                ...
                V= range(self.dist_mat.shape[0])
                Fac = FacilityLocationCIFAR(V, D=self.dist_mat)
                B = math.ceil(budget * len(idxs) / self.N_trn)
                if B == 0:
                    continue
                sset, vals = lazy_greedy_heap(Fac, V, B)
                gamma = np.sum(self.dist_mat < 2.0, axis=1)[sset]
                total_greedy_list.extend(idxs[sset])
                gammas.extend(gamma)
