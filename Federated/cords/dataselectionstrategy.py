...
class DataSelectionStrategy(object):
    ...
    def __init__(self, trainloader, valloader, model, num_classes, linear_layer, loss, device):
        ...
        self.trn_lbls = torch.zeros(self.N_trn).to(dtype=int)
        self.val_lbls = torch.zeros(self.N_val).to(dtype=int)
        self.loss = loss
        self.device = device
    ...
    def get_labels(self, valid=False):
        for batch_idx, (inputs, targets, data_ids) in enumerate(self.trainloader):
            self.trn_lbls[data_ids] = targets

        if valid:
            for batch_idx, (inputs, targets, data_ids) in enumerate(self.valloader):
                self.val_lbls[data_ids] = targets
    ...
    def compute_gradients(self, valid=False, perBatch=False, perClass=False):
        ...
        for batch_idx, (inputs, targets, data_ids) in enumerate(trainloader):
        ...
        if valid:
            for batch_idx, (inputs, targets, data_ids) in enumerate(valloader):
                inputs, targets = inputs.to(self.device), targets.to(self.device, non_blocking=True)
                if batch_idx == 0:
                    out, l1 = self.model(inputs, last=True, freeze=True)
                    loss = self.loss(out, targets).sum()
