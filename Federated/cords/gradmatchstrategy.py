...
from .helpers import OrthogonalMP_REG_Parallel_V1_ED
...
class GradMatchStrategy(DataSelectionStrategy):
    ...
    def ompwrapper(self, X, Y, bud):
        ...
        selected_idxs, weights = OrthogonalMP_REG_Parallel_V1_ED(X, Y, budget=bud,
                                            positive=False, lam=self.lam,
                                            tol=self.eps, device=self.device)

        weights = torch.clamp(weights+1, 1, 5)
        weights = [weights[entry].item() for entry in selected_idxs]
        return torch.LongTensor(selected_idxs).tolist(), weights
    ...
    def select(self, budget, model_params):
        ...
        if self.selection_type == 'PerClass':
            ...
            for i in range(self.num_classes):
                ...
                # This is a case when the client doesnot even have sample of some classes. In such case, it is better to ignore regressing on class altogether.
                if len(trn_subset_idx) < 3:
                    continue
            ...
            for i in range(self.num_classes):
                ...
                if self.valid:
                    ...
                    sum_val_grad = torch.mean(val_gradients, dim=0)#.values#sum(val_gradients, dim=0)
                else:
                    sum_val_grad = torch.mean(trn_gradients, dim=0)#.values#sum(trn_gradients, dim=0)

                idxs_temp, gammas_temp = self.ompwrapper(torch.transpose(trn_gradients, 0, 1),sum_val_grad,
                                                         math.ceil(budget * len(trn_subset_idx) / self.N_trn))
                idxs.extend(list(np.array(trn_subset_idx)[idxs_temp]))
                gammas.extend(gammas_temp)
